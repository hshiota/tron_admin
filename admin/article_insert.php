<?php 
	...
	$mode = array("confirm","return","write","renew");
	$i = 0;
	$max = 4;
	while($i < $max){
	...
	function edit_write($Tpl,$cn,$lang,$select){
		...
		//重複チェック
		$check = check_repeat($post,$cn,$lang,$select,$_SESSION['user_id']);
		if($check == false){
			header("Location:".LIST_PAGE."?year=".$post['year']);
		}
		...
		$result = query_exec($cn,$sql);
		if($result == true){
			$sql = "select id from news where lang = '".$lang."' and category = '".$select."' and user_id = '".$_SESSION['user_id']."' and date_insert = '".$insert_date."'";
			$result = query_exec($cn,$sql);
			$article = result_assoc($result);
			$article_id = $article['id'];//記事ID
					...
					switch($parts_list[$key]['type']){
						...
						case "img":
							if($parts_list[$key]['path'] != ""){
								...
								if(preg_match("/^[0-9]+_.+$/",basename($parts_list[$key]['img_name']))){
									//画像のコピー
									$img_path = img_copy($file_date,$parts_list[$key]['img_name'],$img_filename,$select,$article_id,$lang,$img_cnt);
								}else {
									//画像のアップ
						...
						case "pdf":
							...
							for($i=1;$i<=PARTS_PDF_MAX;$i++){
								if($parts_list[$key]['pdf_text'.$i] != "" && $parts_list[$key]['path'.$i] != ""){
									...
									$pdf_cnt++;
									$sort++;
								}else if($parts_list[$key]['pdf_name'.$i] != "" && $parts_list[$key]['pdf_text'.$i] != "") {
									//ファイル名の取り出し
									...
									if(preg_match("/^[0-9]+_.+$/",basename($parts_list[$key]['pdf_name'.$i]))){
										//PDFのコピー
										$pdf_path = pdf_copy($file_date,$parts_list[$key]['pdf_name'.$i],$pdf_filename,$select,$article_id,$lang,$pdf_cnt);
									}else {
										//PDFのアップ
	...
	//記事を元に新規作成画面
	/*------------------------------------------------------*/
	function edit_renew($Tpl,$cn,$lang,$select){
		if($_POST != ""){
			$post = mb_convert_in($_POST);
		}
...
//			$sql = "select id,lang,category,to_char(date,'yyyy') as year,to_char(date,'mm') as mon,to_char(date,'dd') as day,title,url,url_target,detail_title,to_char(date_open,'yyyy') as open_date_year,to_char(date_open,'mm') as open_date_mon,to_char(date_open,'dd') as open_date_day,to_char(date_open,'hh24') as open_date_hour,to_char(date_open,'mi') as open_date_minute,to_char(date_close,'yyyy') as close_date_year,to_char(date_close,'mm') as close_date_mon,to_char(date_close,'dd') as close_date_day,to_char(date_close,'hh24') as close_date_hour,to_char(date_close,'mi') as close_date_minute,open_flg,top_flg from news where id = ".$post['id'];
			...
			$sql = "select id,lang,category,to_char(date,'yyyy') as year,to_char(date,'mm') as mon,to_char(date,'dd') as day,title,url,url_target,detail_title,to_char(date_open,'yyyy') as open_date_year,to_char(date_open,'mm') as open_date_mon,to_char(date_open,'dd') as open_date_day,to_char(date_open,'hh24') as open_date_hour,to_char(date_open,'mi') as open_date_minute,to_char(date_close,'yyyy') as close_date_year,to_char(date_close,'mm') as close_date_mon,to_char(date_close,'dd') as close_date_day,to_char(date_close,'hh24') as close_date_hour,to_char(date_close,'mi') as close_date_minute,open_flg,top_flg,news_category from news where id = ".$post['id'];
			$result = query_exec($cn,$sql);
		...
		if(is_array($parts_list)){
			uasort($parts_list,"sort_new");
			$j = 1;
			$now_parts_cnt = count($parts_list);
			foreach($parts_list as $key => $val){
				if($parts_list[$key]['type'] != ""){
					$assign_array = array();
					if($post['mode_parts_delete'.$key] == ""){
